...
const checkForAdsReadyToRefresh = () => {
	...
	if (slotsToRefresh.length > 0) {
		if (refreshCallback && typeof window[refreshCallback] === 'function') {
...
const isEligible = (event) => {
	...
	if (typeof advertiserIds[slotInfo.advertiserId] !== 'undefined') {
		return false;
	}

	// Prevent a refresh if the line item ID is blacklisted.
	if (typeof lineItemIds[slotInfo.lineItemId] !== 'undefined') {
		return false;
	}

	// Fluid returns 0x0 - ensure fluid is supported on the unit too.
	if (slotSize === '0,0' && slotSizes.includes('fluid')) {
		slotSize = 'fluid';
	}
	...
	if (typeof slotIdsToExclude[slotID] !== 'undefined') {
		return false;
	}

	// Enforce limit on maximum number of refreshes per slot.
	if (typeof adsData[slotID] !== 'undefined' && adsData[slotID].renderCount >= maximumRefreshes) {
		return false;
	}
...
const initializeSlotData = (event) => {
	...
	if (typeof adsData[slotID] !== 'undefined') {
		return;
	}
